...
public class TransactionRepo implements TransactionDAO {
  ...
  public void addEventIdForTrxn(String transactionId, int eventId) {
    String sql = "UPDATE Transaction SET eventId = ? " +
        "WHERE transactionId = ?";
  ...
  public List<Map<String, Object>> getAll(int userId) {
    ...
    String sql = "SELECT t.transactionId , t.date , t.time , t.amount, e.name as eventName,sb.seatId,v.name as venueName FROM Transaction t , Event e, SeatBook sb,Venue v WHERE userId= ?  AND e.eventId = t.eventId AND sb.transactionId = t.transactionId  AND v.venueId in (SELECT tp.venueId FROM TakesPlace tp WHERE t.eventId = tp.eventId) ;";

    return res = jdbcTemplate.queryForList(sql, userId);
  }
